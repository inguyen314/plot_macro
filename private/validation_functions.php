<?php
  ...
  function has_unique_page_menu_name($db, $menu_name, $current_id="0") {
	$page_count= '';
	  
    try 
    {		
        $sql = "SELECT * FROM WM_MVS_CF.PAGES ";
        $sql .= "WHERE MENU_NAME='" . $menu_name . "' ";
        $sql .= "AND ID != '" . $current_id . "'";		

        echo '<hr> $sql @ has_unique_page_menu_name @ validation_function: <br>';
		var_dump($sql);
		...
		$row = oci_fetch_row($stmnt_query);
		
		var_dump($row);
		
		$page_count = intval($row[0]);
  ...
  function has_unique_username($db, $username, $current_id="0") {
  $admin_count= '';

	try 
	{		
		$sql = "SELECT count(ID) FROM WM_MVS_CF.ADMINS ";
		$sql .= "WHERE USERNAME='" . $username . "' ";
		$sql .= "AND id != '" . $current_id . "'";
		
		echo '<hr> $sql @ has_unique_username @ validation_function: <br>';
		var_dump($sql);
